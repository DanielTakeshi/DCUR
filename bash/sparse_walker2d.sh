...
for N_ALPHA in 1 ; do
    NAME=walker2d_td3_offline_curriculum_ep_${EPOCHS}_${TYPE}_p_${PREV}_n_${NEXT}_overlap_sparse_tp_${N_ALPHA}
    ...
    NAME=walker2d_td3_online_stud_total_${STUDENT_SIZE}_curriculum_ep_${EPOCHS}_${TYPE}_p_${PREV}_n_${NEXT}_overlap_sparse_tp_${N_ALPHA}
    for SEED in 30 31 32 ; do
...
for N_ALPHA in 1 ; do
    NAME=walker2d_td3_offline_curriculum_ep_${EPOCHS}_${TYPE}_scale_${C_SCALE}t_overlap_sparse_tp_${N_ALPHA}
    for SEED in 30 31 32 ; do
        taskset -c ${CPU1}-${CPU2} python spinup/teaching/offline_rl.py  \
    ...
    NAME=walker2d_td3_online_stud_total_${STUDENT_SIZE}_curriculum_ep_${EPOCHS}_${TYPE}_scale_${C_SCALE}t_overlap_sparse_tp_${N_ALPHA}
    for SEED in 30 31 32 ; do
        taskset -c ${CPU1}-${CPU2} python spinup/teaching/online_rl.py  \
            --env ${ENV}  --exp_name ${NAME}  --seed ${SEED}  -bp ${B_PATH}  --t_source ${T_SOURCE}  --curriculum ${TYPE}  --c_scale ${C_SCALE} \
            --n_alpha ${N_ALPHA}  -tp ${T_PATH}  --epochs ${EPOCHS}  --overlap  --sparse  --student_size ${STUDENT_SIZE}
