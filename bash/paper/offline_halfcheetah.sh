...
# NOTE: don't have the ones that decrease PREV (yet), whalfcheetah to find best ones here first.
# ------------------------------------------------------------------------------ #

PREV=1000000
for NEXT in 50000 100000 200000 500000 1000000 ; do
    NAME=halfcheetah_td3_offline_curriculum_ep_${EPOCHS}_${TYPE}_p_${PREV}_n_${NEXT}_overlap
...
for C_SCALE in 0.50 0.75 1.00 1.10 ; do
    NAME=halfcheetah_td3_offline_curriculum_ep_${EPOCHS}_${TYPE}_scale_${C_SCALE}t_overlap
    for SEED in 90 91 92 93 94 ; do
        taskset -c ${CPU1}-${CPU2} python spinup/teaching/offline_rl.py  \
            --env ${ENV}  --exp_name ${NAME}  --seed ${SEED}  -bp ${B_PATH}  --t_source ${T_SOURCE} \
